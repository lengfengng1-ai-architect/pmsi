...
| 里程碑 | 推/控 | **P0** | go/no-go、决策阻塞、否决挂载（一等对象） |
...
|------|------|---------|----------|:---:|
| **P0 · MVP** | 单项目跑通拆控推追回全周期 | 立项/WBS、任务闭环、基线+变更(轻)、风险、决策管理+升级、自动化(核心)、复盘、健康分(轻)、里程碑 | PM 用完说「比微信群+Excel 强，因为它逼我闭环和决策」 | ≈ 8–12 周 |
...
6. **简易健康分（轻）** — 见下。
7. **里程碑（推/控）** — go/no-go、决策阻塞、否决挂载（一等对象）。
8. **结构化复盘（回）** — 收尾产出 ≥1 条可复用经验。

> 这 8 步就是 MVP 的关键路径，前一步是后一步的数据前提。

### 4.3 MVP 健康分简化版

不做完整 8 维 + EVM。MVP 用 4 个已有信号算粗粒度红黄绿：
- 进度完成偏差（实际完成% − 计划完成%，非 SPI；SPI 属 P1 EVM）
- 逾期任务数
- 阻塞/逾期决策数
- 高危未关风险数

+ P0 否决（与 docs/04 §四、AC-HS-LIGHT-02 一致）：已实现高危风险无应对、基线非受控突破、阻塞里程碑的决策逾期；关键路径滑移 / CPI·SPI 否决依赖 CPM/EVM，MVP 显示"未启用"。明确标注「数据不全」，P1 上完整模型。
...
> **MVP 范围裁定（回应 docs/13）**：① **移动端**含最小 PWA（决策拍板/任务确认/审批，决策 SLA 依赖），原生 App = P2；② **报表**含最小自动状态报告（基于健康分下钻，否则 PM 回 Excel 双轨），完整仪表盘 = P2；③ **工作日历**含默认组织工作日历（节假日，SLA 按工作日算），项目/个人日历 = P1；④ **通知中心**为系统服务（非一等对象），MVP 站内+邮件，IM 双向 = P1。
